...
The S3 objejct `results` contains a grid of estimates, standard errors and one-sided confidence intervals ranging from `p = 0` to `p = p_upper'. 
...
The point and confidence interval estimates of the `cucc' command is based on the scale of log relative risk. It is more conventional to look at the results in terms of the relative scale. To do so, we take the exponential:  
